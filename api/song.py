...
def list_all_songs():
    try:
        ...
        oldsongs = session.query(Song).all()
        songs=[]
        for i in oldsongs:
            if i.user_id!=1:
                songs.append(i)
...
def search_songs():
    try:
        ...
        oldsongstitle = session.query(Song).filter(Song.title.contains(keyword)).all()

        songstitle = []
        for i in oldsongstitle:
            if i.user_id != 1:
                songstitle.append(i)

        songs = songstitle
        for i in songsartist:
            if i not in songs and i.user_id != 1:
                songs.append(i)
