...
def edit():
    ...
    try:
        ...
        if not None in responses and "Not changed" in responses:
            for i, j in enumerate(responses):
                if j != "Not changed":
                    ...
                    notchanged += " " + type[i]
            return jsonResponse('Error', 'Nothing changed, already up-to-date: '+ notchanged + '| bad input: '+ error, 403)
        ...
        return jsonResponse('Success','Changed: '+ changed + '| already up-to-date: '+ notchanged + '| bad input: '+ error, 207)
